...
/// visible frame.
///
/// This is the render seam the spec places in the domain core: it uses **CoreGraphics /
/// CoreText / ImageIO** — pure, screen-free, permission-free C frameworks — and imports
/// **no AppKit or ScreenCaptureKit**, so the pixel-level assertions in the test suite run
/// without a display. Element geometry arrives in image pixel coordinates (top-left
/// origin, y down); the context is set up bottom-left/y-up, so every coordinate is mapped
/// through `contextPoint` / `contextRect` rather than flipping the whole context (which
/// would mirror the base image and the text).
...
private struct Flattener {
    ...
    private func drawArrowhead(from: Point, to: Point, style: Style) {
        ...
        let headLength = max(style.strokeWidth * 3.5, 10)
        ...
        context.beginPath()
        context.move(to: tip)
        context.addLine(to: CGPoint(x: baseX + px * headWidth, y: baseY + py * headWidth))
        context.addLine(to: CGPoint(x: baseX - px * headWidth, y: baseY - py * headWidth))
        context.closePath()
        context.fillPath()
    }
