...
- **Workflow:** spec → Linear (`ready-for-agent`) → branch per issue → PR to `main` → Copilot review loop (fix → reply → resolve → re-request). PR titles use **`<type>(LIG-<n>): <title>`** (e.g. `feat(LIG-100): …`). See AGENTS.md → *The main workflow*.
