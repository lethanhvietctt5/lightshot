...
- **`AppCoordinator` composes, and ordering matters:** for area/window capture (and OCR Text) the screen **freezes first** — `CaptureService.freezeScreen()` stills every display — *then* the overlay runs over those stills to resolve a `CaptureRegion` (in global top-left screen points), and the result is taken from the `FrozenScreen`, not captured live (spec 0011). Only the self-timer paths capture live after the overlay. Fullscreen skips the overlay; open-file goes through `ImageSource`. All three converge on one `openEditor(with: CapturedImage)` entry. The post-capture toolbar is a step *after* the image exists, not part of the selection overlay. `CaptureError.permissionDenied` routes to a System-Settings recovery path, never a blank editor.
