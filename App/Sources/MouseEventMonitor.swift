...
/// The OS side of `InputEventSource` for the mouse (spec 0006, story 29): global event monitors
/// for pointer moves, drags and button presses. Global monitors for mouse events need no grant —
/// only keyboard monitors are gated by Input Monitoring (R11). Positions are converted from
/// AppKit's bottom-left global coordinates to the top-left screen points every `CaptureRegion` uses.
