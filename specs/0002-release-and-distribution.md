...
5. **Verify.** `codesign --verify --strict` passes; the output of `codesign -d -r-` equals `scripts/release-identity.txt`, compared case-insensitively since `codesign` prints the certificate hash in lowercase hex while `security find-identity` shows it in uppercase (story 70); `lipo -archs` reports both architectures; the built `Info.plist` reports `<version>`. `spctl --assess` is run for information only — it is *expected* to reject a self-signed app.
