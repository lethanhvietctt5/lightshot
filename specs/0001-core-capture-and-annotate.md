...
24. As a user, I want redaction to be **flattened into the exported image** (not a removable overlay), so that hidden content can't be recovered from the file I share.
...
- **`AnnotationDocument` (primary domain module, the primary test seam).** A value type holding: a reference/handle to the immutable base image, its pixel size, an ordered array of `AnnotationElement`s, the current selection, and a crop rect. All mutation happens through an explicit command API rather than direct property mutation:
  ...
  - `AnnotationElement` is an enum/struct-with-kind covering: `arrow`, `line`, `rectangle`, `ellipse`, `freehand(points)`, `text(string)`, `highlight(rect)`, `redaction(rect, style: blur|pixelate)`, `stepMarker(number)`. Each carries geometry (in **image coordinates**, not view coordinates) plus a `Style` (color, stroke width, font size, fill).
...
- **`render(_ document) -> RenderedImage` (supporting pure-function seam).** Deterministic flattening of base image + elements (respecting crop and z-order) into a bitmap via Core Graphics / `ImageRenderer`. Redaction elements are rasterized destructively so underlying pixels are gone (story 24/44). Output size equals the crop rect size at native scale.

- **`ImageSink` protocol.** `write(_ image, to:URL, format:)`, `copyToClipboard(_ image)`, plus a drag-provider. Real implementation uses `NSPasteboard` / file APIs; a fake captures calls in tests. Keeps disk/clipboard out of the render tests.
...
- **OS-facing controllers behind protocols (thin, protocol-fronted, not the unit-test focus):**
  - `CaptureService` — wraps ScreenCaptureKit (`SCShareableContent`, `SCScreenshotManager` / stream) for area/window/fullscreen; resolves the target display; honors cursor-inclusion and self-timer. Exposes `authorizationStatus` and a `requestAuthorization()` flow.
  ...
  - `OverlayController` — the full-screen selection overlay window: dimming, live selection rect with dimension readout, edge/corner handles, window-hover highlighting, Escape-to-cancel, and the post-capture action toolbar.
...
- **Composition.** A thin `AppCoordinator` / menu-bar controller wires hotkey → capture → (overlay) → document → editor window, and editor output → `ImageSink` / `PinBoardController` / `HistoryStore`. The coordinator holds protocol references so the domain core never imports AppKit capture APIs.
...
- **Redaction is destructive on export**, never a maskable overlay in the output (security requirement, story 24).
...
- **Modules under test:**
  ...
  - **`render` (supporting):** output size equals crop rect at native scale; redaction destroys underlying pixels; element z-order reflected; empty document renders the base image unchanged in size.
  ...
  - **`ImageSink` fake:** used to verify the editor's output path calls copy/save with the rendered image — behavior of the coordinator's output step, not the OS.
...
- **Publishing note:** this spec could not be posted to the issue tracker — the Linear connector was not authorized in this session and no triage-label vocabulary was provided. It is saved here as a Markdown file; push it to the tracker (with the `ready-for-agent` label) manually, or authorize Linear and ask me to publish it.
