...
- **After a take, the file waits in scratch until the overlay decides.** *As built (R12):* `RecordingDefaults.afterRecording` (show overlay — the default — / save silently / open editor, story 33) is read when the take finishes. The overlay path leaves the file in the scratch directory and hands the coordinator a `PendingRecording`; **Save** — or a dismissal by Escape, the 20-second timeout, quitting, or a new take starting — moves it to the save location under the filename pattern or the name typed into the overlay's Rename field (`/` and `:` become `-`, surrounding whitespace and dots go, an empty name falls back to the pattern), **Copy file** saves the same way and puts the saved file's URL on the pasteboard (a scratch URL would dangle once the overlay moved the file), **Delete** confirms then moves it to the Trash, and the preview drags out as the file itself (a copy; the original is still saved on dismissal). Save silently and open editor move the file first; the editor and Trim are shown disabled until R14, and "open the video editor" reveals the saved file in Finder until then. A failed save or delete surfaces and leaves the take pending, so nothing is lost.
