...
| R3 | LIG-29 | Recording region: overlay *record* mode — editable selection (handles, move, arrow keys, no confirm on mouse-up), aspect-ratio lock, typed size, window pick, remember-last-area; `CaptureRegion.display` | 3, 4, 5, 6, 7 | R2 | Extends `OverlayController` / `SelectionOverlayModel`; no new seam |
